...
// SentryWebReconciler reconciles SentryWeb for SentryCluster
type SentryWebReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// Reconcile handles the Sentry Web deployment for Sentry.
func (r *SentryWebReconciler) Reconcile(ctx context.Context, sentryCluster *sentryv1alpha1.SentryCluster) (ctrl.Result, error) {
	log := log.FromContext(ctx)
	log.Info("Reconciling Sentry Web", "SentryCluster", sentryCluster.Name)
	
	// Check if dependencies are ready
	if !r.areDependenciesReady(sentryCluster) {
		log.Info("Dependencies for Sentry Web are not ready yet, requeuing")
		return ctrl.Result{RequeueAfter: time.Second * 30}, nil
	}

	// 1. Reconcile Service
	serviceName := sentryCluster.Name + "-web"
	service := &corev1.Service{}
	err := r.Get(ctx, types.NamespacedName{Name: serviceName, Namespace: sentryCluster.Namespace}, service)
	if err != nil && apierrors.IsNotFound(err) {
		desiredService := r.defineSentryWebService(sentryCluster)
		log.Info("Creating a new Sentry Web Service", "Service.Namespace", desiredService.Namespace, "Service.Name", desiredService.Name)
		if err := r.Create(ctx, desiredService); err != nil {
			log.Error(err, "Failed to create new Sentry Web Service", "Service.Namespace", desiredService.Namespace, "Service.Name", desiredService.Name)
			return ctrl.Result{}, errors.Wrap(err, "failed to create Sentry Web Service")
		}
		return ctrl.Result{Requeue: true}, nil
	} else if err != nil {
		log.Error(err, "Failed to get Sentry Web Service")
		return ctrl.Result{}, errors.Wrap(err, "failed to get Sentry Web Service")
	} else {
		log.V(1).Info("Sentry Web Service already exists", "Service.Namespace", service.Namespace, "Service.Name", service.Name)
	}

	// 2. Reconcile Deployment
	deploymentName := sentryCluster.Name + "-web"
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		log.Info("Creating a new Sentry Web Deployment", "Deployment.Namespace", desiredDeployment.Namespace, "Deployment.Name", desiredDeployment.Name)
		if err := r.Create(ctx, desiredDeployment); err != nil {
			log.Error(err, "Failed to create new Sentry Web Deployment", "Deployment.Namespace", desiredDeployment.Namespace, "Deployment.Name", desiredDeployment.Name)
			return ctrl.Result{}, errors.Wrap(err, "failed to create Sentry Web Deployment")
		}
		return ctrl.Result{Requeue: true}, nil
	} else if err != nil {
		log.Error(err, "Failed to get Sentry Web Deployment")
		...
		if !reflect.DeepEqual(deployment.Spec, desiredDeployment.Spec) {
			log.Info("Updating existing Sentry Web Deployment", "Deployment.Namespace", deployment.Namespace, "Deployment.Name", deployment.Name)
			deployment.Spec = desiredDeployment.Spec
			if err := r.Update(ctx, deployment); err != nil {
				log.Error(err, "Failed to update Sentry Web Deployment", "Deployment.Namespace", deployment.Namespace, "Deployment.Name", deployment.Name)
				return ctrl.Result{}, errors.Wrap(err, "failed to update Sentry Web Deployment")
			}
			return ctrl.Result{Requeue: true}, nil
		}
	}

	log.Info("Sentry Web reconciled successfully", "SentryCluster", sentryCluster.Name)
...
func (r *SentryWebReconciler) defineSentryWebService(sentryCluster *sentryv1alpha1.SentryCluster) *corev1.Service {
	...
	if err := controllerutil.SetControllerReference(sentryCluster, svc, r.Scheme); err != nil {
		log.FromContext(context.Background()).Error(err, "Failed to set controller reference on Sentry Web Service")
	}
	return svc
}
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{{
						...
						Resources: sentryCluster.Spec.Resources.Web,
...
// areDependenciesReady checks if all dependencies for Sentry Web are ready
func (r *SentryWebReconciler) areDependenciesReady(sentryCluster *sentryv1alpha1.SentryCluster) bool {
	...
	// Check if Symbolicator is ready
